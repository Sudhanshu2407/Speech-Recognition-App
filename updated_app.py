...
def process_command(cmd):
    if 'play' in cmd:
        ...
        webbrowser.open("https://nareshit.com/")
...
# Run the voice assistant with a break between queries
if __name__ == "__main__":
    while True:
        command = hear()
        if command:
            process_command(command)
            time.sleep(2)  # Take a small break before asking for the next command
        else:
